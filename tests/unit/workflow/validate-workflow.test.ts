...
describe("Validate Workflow Restrictions", () => {
  ...
  it("accepts agent() with provider-neutral skills, context, workspace, and handoff", () => {
    const parsed = createParsed(`
      await agent({
        id: "review-methodology",
        provider: "pi",
        model: "deepseek-v4-pro",
        prompt: "Review the methodology section.",
        skills: ["skills/paper-review/SKILL.md"],
        context: {
        ...
        handoff: {
          ...
          instructions: "Summarize findings and uncertainty.",
          required: true
        }
      });
    `);
  ...
  it("flags expectedOutputs on agent() because handoff is the supported contract", () => {
    ...
    `);
    const issues = validateWorkflow(parsed, options);
    expect(issues.some(i => i.message.includes("expectedOutputs"))).toBe(true);
  });

  it("flags malformed additive agent fields", () => {
    const parsed = createParsed(`
      await agent({
        prompt: "hello",
        skills: ["skills/review/SKILL.md", ""],
        context: { files: "input.md", handoff: [123], notes: false },
        workspace: { cwd: "", mode: "remote" },
        handoff: { writeTo: 123, instructions: false, required: "yes" }
      });
    `);
    const issues = validateWorkflow(parsed, options);
    const messages = issues.map(i => i.message);

    expect(messages.some(message => message.includes("skills entries must be non-empty string literals"))).toBe(true);
    expect(messages.some(message => message.includes("context.files must be an array of string literals"))).toBe(true);
