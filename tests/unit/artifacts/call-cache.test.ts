...
describe("call cache", () => {
  ...
  it("computes agent fingerprints including skills, context, workspace, and handoff contracts", () => {
    const base = {
      call: {
        id: "a",
        prompt: "hello",
    ...
    const first = computeAgentFingerprint(base);
    expect(computeAgentFingerprint({ ...base })).toBe(first);
    expect(computeAgentFingerprint({
      ...base,
      call: { ...base.call, skills: ["skills/other/SKILL.md"] }
    })).not.toBe(first);
    expect(computeAgentFingerprint({
      ...base,
      call: { ...base.call, context: { ...base.call.context, files: ["input/other.md"] } }
    })).not.toBe(first);
    expect(computeAgentFingerprint({
      ...base,
    ...
    expect(computeAgentFingerprint({
      ...base,
      call: { ...base.call, handoff: { ...base.call.handoff, required: false } }
    })).not.toBe(first);
  });
