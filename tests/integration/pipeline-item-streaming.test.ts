...
const TEMP_DIR = path.resolve("tests/temp-pipeline-streaming");
...
describe("Pipeline item-streaming integration", () => {
  beforeEach(async () => {
  ...
  it("item-streaming allows independent advancement", async () => {
    ...
    const idxItem2S1Completed = logMessages.indexOf("stage1 completed for item2");
    ...
    expect(idxItem2S1Completed).toBeGreaterThan(-1);
    expect(idxItem2S2Started).toBeGreaterThan(-1);
    expect(idxItem1S1Completed).toBeGreaterThan(-1);

    expect(idxItem2S1Completed).toBeLessThan(idxItem1S1Completed);
    expect(idxItem2S2Started).toBeLessThan(idxItem1S1Completed);
  });
});
