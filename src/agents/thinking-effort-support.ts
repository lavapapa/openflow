...
const SUPPORTED_PROVIDERS = new Set(["codex", "pi", "pi-sdk", "opencode"]);
...
export function assertThinkingEffortSupported(provider: string, value?: ThinkingEffort): void {
  ...
  if (!SUPPORTED_PROVIDERS.has(provider)) {
    throw new OpenDynamicWorkflowError(
      ...
      `Provider '${provider}' does not support thinkingEffort. Supported providers: codex, pi, pi-sdk, opencode.`
    );
  }
  ...
  } else if (provider === "pi" || provider === "pi-sdk") {
    if (!PI_THINKING_EFFORTS.has(value)) {
      throw new OpenDynamicWorkflowError(
        ErrorCode.THINKING_EFFORT_VALUE_UNSUPPORTED,
        `Provider '${provider}' does not support thinkingEffort '${value}'. Supported values: off, minimal, low, medium, high, xhigh.`
      );
    }
