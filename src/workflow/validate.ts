...
export function validateWorkflow(
  ...
  function validateAgentCall(node: ts.CallExpression, isContextForm: boolean, contextName: string = "ctx") {
    ...
    const isStringLiteralLike = (expr: ts.Expression) => ts.isStringLiteral(expr) || ts.isNoSubstitutionTemplateLiteral(expr);

    const validateStringLiteralArray = (expr: ts.Expression, label: string, requireNonEmpty: boolean) => {
      if (!ts.isArrayLiteralExpression(expr)) {
        if (isStaticValue(expr)) {
          report(expr, `${callPrefix} ${label} must be an array of string literals.`);
        }
        return;
      }
      for (const element of expr.elements) {
        if (!isStringLiteralLike(element)) {
          if (isStaticValue(element)) {
            report(element, `${callPrefix} ${label} entries must be string literals.`);
          }
        } else if (requireNonEmpty && element.text.trim() === "") {
          report(element, `${callPrefix} ${label} entries must be non-empty string literals.`);
        }
      }
    };

    const validateContextLiteral = (expr: ts.Expression) => {
      if (!ts.isObjectLiteralExpression(expr)) {
        if (isStaticValue(expr)) {
          report(expr, `${callPrefix} context must be an object literal.`);
        }
        return;
      }

      for (const contextProp of expr.properties) {
        if (!ts.isPropertyAssignment(contextProp)) {
          report(contextProp, `${callPrefix} context does not support shorthand, spread, or method properties.`);
          continue;
        }
        if (!ts.isIdentifier(contextProp.name) && !ts.isStringLiteral(contextProp.name)) {
          report(contextProp.name, `${callPrefix} context does not support computed property names.`);
          continue;
        }
        const key = contextProp.name.text;
        const init = contextProp.initializer;
        if (key === "files") {
          validateStringLiteralArray(init, "context.files", true);
        } else if (key === "handoff") {
          if (isStringLiteralLike(init)) {
            if (init.text.trim() === "") {
              report(init, `${callPrefix} context.handoff must be non-empty when provided.`);
            }
          } else if (ts.isArrayLiteralExpression(init)) {
            for (const element of init.elements) {
              if (!isStringLiteralLike(element)) {
                if (isStaticValue(element)) {
                  report(element, `${callPrefix} context.handoff entries must be string literals.`);
                }
              } else if (element.text.trim() === "") {
                report(element, `${callPrefix} context.handoff entries must be non-empty string literals.`);
              }
            }
          } else if (isStaticValue(init)) {
            report(init, `${callPrefix} context.handoff must be a string literal or an array of string literals.`);
          }
        } else if (key === "notes") {
          if (!isStringLiteralLike(init) && isStaticValue(init)) {
            report(init, `${callPrefix} context.notes must be a string literal.`);
          }
        } else {
          report(contextProp.name, `${callPrefix} context contains unsupported key '${key}'.`);
        }
      }
    };

    const validateWorkspaceLiteral = (expr: ts.Expression) => {
      if (!ts.isObjectLiteralExpression(expr)) {
        if (isStaticValue(expr)) {
          report(expr, `${callPrefix} workspace must be an object literal.`);
        }
        return;
      }
      ...
      for (const workspaceProp of expr.properties) {
        ...
        if (!ts.isIdentifier(workspaceProp.name) && !ts.isStringLiteral(workspaceProp.name)) {
          report(workspaceProp.name, `${callPrefix} workspace does not support computed property names.`);
          continue;
        }
        const key = workspaceProp.name.text;
        const init = workspaceProp.initializer;
        if (key === "cwd") {
          if (isStringLiteralLike(init)) {
            if (init.text.trim() === "") {
              report(init, `${callPrefix} workspace.cwd must be a non-empty string literal.`);
            }
          } else if (isStaticValue(init)) {
            report(init, `${callPrefix} workspace.cwd must be a non-empty string literal.`);
          }
    ...
    const validateHandoffLiteral = (expr: ts.Expression) => {
      if (!ts.isObjectLiteralExpression(expr)) {
        if (isStaticValue(expr)) {
          report(expr, `${callPrefix} handoff must be an object literal.`);
        }
        return;
      }
      for (const handoffProp of expr.properties) {
        if (!ts.isPropertyAssignment(handoffProp)) {
          report(handoffProp, `${callPrefix} handoff does not support shorthand, spread, or method properties.`);
          continue;
        }
        if (!ts.isIdentifier(handoffProp.name) && !ts.isStringLiteral(handoffProp.name)) {
          report(handoffProp.name, `${callPrefix} handoff does not support computed property names.`);
          continue;
        }
        const key = handoffProp.name.text;
        const init = handoffProp.initializer;
        if (key === "writeTo") {
          if (isStringLiteralLike(init)) {
            if (init.text.trim() === "") {
              report(init, `${callPrefix} handoff.writeTo must be a non-empty string literal.`);
            }
          } else if (isStaticValue(init)) {
            report(init, `${callPrefix} handoff.writeTo must be a string literal.`);
          }
        } else if (key === "instructions") {
          if (!isStringLiteralLike(init) && isStaticValue(init)) {
            report(init, `${callPrefix} handoff.instructions must be a string literal.`);
          }
        } else if (key === "required") {
          if (init.kind !== ts.SyntaxKind.TrueKeyword && init.kind !== ts.SyntaxKind.FalseKeyword && isStaticValue(init)) {
            report(init, `${callPrefix} handoff.required must be a boolean literal.`);
          }
        } else {
          report(handoffProp.name, `${callPrefix} handoff contains unsupported key '${key}'.`);
        }
      }
    };
    ...
    for (const prop of firstArg.properties) {
      if (ts.isSpreadAssignment(prop)) {
        ...
        if (propName === "expectedOutputs") {
          report(prop, `${callPrefix} expectedOutputs is not supported. Use handoff.writeTo for soft output verification.`);
        } else if (propName === "skills" && ts.isPropertyAssignment(prop)) {
          validateStringLiteralArray(prop.initializer, "skills", true);
        } else if (propName === "context" && ts.isPropertyAssignment(prop)) {
          validateContextLiteral(prop.initializer);
        } else if (propName === "workspace" && ts.isPropertyAssignment(prop)) {
          validateWorkspaceLiteral(prop.initializer);
        } else if (propName === "handoff" && ts.isPropertyAssignment(prop)) {
          validateHandoffLiteral(prop.initializer);
        } else if (propName === "permissions" && ts.isPropertyAssignment(prop)) {
          const init = prop.initializer;
