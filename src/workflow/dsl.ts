...
function normalizeStringList(value: unknown, label: string): string[] {
  if (!Array.isArray(value)) {
    throw new InvalidDslCallError(`agent() ${label} must be an array of non-empty strings.`);
  }
  for (const item of value) {
    if (typeof item !== "string" || item.trim() === "") {
      throw new InvalidDslCallError(`agent() ${label} entries must be non-empty strings.`);
    }
  }
  return [...value];
}

function normalizeOptionalPathList(value: unknown, label: string): string[] | undefined {
  if (value === undefined) return undefined;
  if (typeof value === "string") {
    if (value.trim() === "") {
      throw new InvalidDslCallError(`agent() ${label} must be a non-empty string or an array of non-empty strings.`);
    }
    return [value];
  }
...
function normalizeAgentContext(value: unknown): DirectAgentCallInput["context"] {
  if (value === undefined) return undefined;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new InvalidDslCallError("agent() context must be an object.");
  }

  const typed = value as Record<string, unknown>;
  const context: NonNullable<DirectAgentCallInput["context"]> = {};

  if (typed.files !== undefined) {
    context.files = normalizeStringList(typed.files, "context.files");
  }
  if (typed.handoff !== undefined) {
    context.handoff = normalizeOptionalPathList(typed.handoff, "context.handoff");
  }
  if (typed.notes !== undefined) {
    if (typeof typed.notes !== "string") {
      throw new InvalidDslCallError("agent() context.notes must be a string.");
    }
    context.notes = typed.notes;
  }

  return context;
}

function normalizeAgentHandoff(value: unknown): DirectAgentCallInput["handoff"] {
  if (value === undefined) return undefined;
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new InvalidDslCallError("agent() handoff must be an object.");
  }

  const typed = value as Record<string, unknown>;
  const handoff: NonNullable<DirectAgentCallInput["handoff"]> = {};

  if (typed.writeTo !== undefined) {
    if (typeof typed.writeTo !== "string" || typed.writeTo.trim() === "") {
      throw new InvalidDslCallError("agent() handoff.writeTo must be a non-empty string.");
    }
    handoff.writeTo = typed.writeTo;
  }
  if (typed.instructions !== undefined) {
    if (typeof typed.instructions !== "string") {
      throw new InvalidDslCallError("agent() handoff.instructions must be a string.");
    }
    handoff.instructions = typed.instructions;
  }
  if (typed.required !== undefined) {
    if (typeof typed.required !== "boolean") {
      throw new InvalidDslCallError("agent() handoff.required must be a boolean.");
    }
    handoff.required = typed.required;
  }
...
function buildAgentAttachmentMetadata(input: {
  ...
  if (input.skills) metadata.skillCount = input.skills.length;
  if (input.context?.files) metadata.contextFileCount = input.context.files.length;
  if (input.context?.handoff) {
...
export function createDsl(runtime: RuntimeState) {
  ...
  const runDirectAgent = async (input: DirectAgentCallInput): Promise<AgentResult> => {
    ...
    if ((input as any).expectedOutputs !== undefined) {
      throw new InvalidDslCallError("agent() expectedOutputs is not supported. Use handoff.writeTo for soft output verification.");
    }
    const normalizedSkills = input.skills !== undefined ? normalizeStringList(input.skills, "skills") : undefined;
    const normalizedContext = normalizeAgentContext(input.context);
    const normalizedHandoff = normalizeAgentHandoff(input.handoff);
    const normalizedWorkspace = normalizeWorkspaceInput(input, runtime.cwd);
    if (
    ...
    const fingerprint = computeAgentFingerprint({
      call: {
        ...
        ...(input.workspace !== undefined ? { workspace: normalizedWorkspace } : {})
      },
    ...
    const task: ScheduledTask<AgentResult> = {
      ...
      metadata: {
        ...input.metadata,
        ...buildAgentAttachmentMetadata({
          skills: normalizedSkills,
          context: normalizedContext,
          workspace: input.workspace !== undefined ? normalizedWorkspace : undefined,
      ...
      run: async (schedulerSignal: AbortSignal) => {
        ...
        if (normalizedSkills !== undefined) execInput.skills = normalizedSkills;
        if (normalizedContext !== undefined) execInput.context = normalizedContext;
        if (input.workspace !== undefined) execInput.workspace = normalizedWorkspace;
        if (normalizedHandoff !== undefined) execInput.handoff = normalizedHandoff;
