...
export function validateConfig(config: OpenDynamicWorkflowConfig): void {
  ...
  for (const [name, provider] of Object.entries(config.providers)) {
    ...
    const toolArrays = ["safeTools", "fullAccessTools", "tools", "excludeTools"];
    ...
    if (provider.maxContextChars !== undefined && (!Number.isInteger(provider.maxContextChars) || provider.maxContextChars < 1)) {
      throw new OpenDynamicWorkflowError(
        ErrorCode.CONFIG_VALIDATION_ERROR,
        `Provider '${name}' maxContextChars must be a positive integer.`
      );
    }
  }
